# 更新日志

本项目遵循语义化版本号：`主版本.功能版本.修复版本`。

## 未发布

暂无。

## v0.2.1 - 2026-08-13

### 新增

- 新增 Apple TV 本机动态网页解析，并在静态解析失败后自动拦截网页产生的媒体地址。
- 新增 App 内 `127.0.0.1` HLS/MP4 播放代理，支持播放列表、分片、密钥、Range 与来源请求头。
- 新增 Kazumi API 8 JSON 规则支持，可通过公开 JSON API 搜索条目并生成线路与播放页。
- Apple TV Top Shelf 支持在设置中选择当季新番、最近更新、用户追番或播放历史，默认显示最近更新。

### 变更

- 应用图标、启动页和 Top Shelf 统一更换为与 App 一致的黑绿底与主题绿 `K`/播放融合标识，提高电视主屏小尺寸下的辨识度。
- 启动图与 tvOS 应用图标改用全新资产名称，避免系统沿用旧版同名资产缓存。
- 新增 Apple TV 主屏 Top Shelf 内容扩展，聚焦 App 图标时显示番组轮播；本地来源无数据或网络失败时安全回退，不再出现空白区域。
- Top Shelf 的四种内容来源统一屏蔽 Bangumi 标记为中国制作的动画；旧版未标记制作地区的本地轮播缓存会等待重新识别，不会直接展示。
- 播放不再依赖用户部署 Python/Playwright 服务。
- 移除仓库中的 Python/Playwright 外部解析服务。
- 设置页移除外部服务器地址与连接测试，动态网页解析默认开启。
- 默认规则调整为本轮实播全部通过的 `TvTFun` 与 `xfdmneo`，升级时自动移除旧版预装的失效规则。
- 去后端化后统一规则能力标签为本地和需验证，并在规则管理页隐藏弃用规则；迁移仅清理明确弃用规则，并保留本项目端侧验证通过的 `TvTFun` 兼容例外。
- 弹幕默认字体大小由 18 调整为 24，并兼容迁移已有安装中的旧默认值。

### 修复

- Top Shelf 扩展移除侧载签名通常无法授权的固定 App Group entitlement，避免重签后扩展被 tvOS 忽略；无共享缓存时在 1.5 秒内直连最近更新，超时立即显示本地兜底图。
- Top Shelf 改为由主 App 预取并缓存远程目录，桌面扩展同步返回内容，避免真机网络较慢时顶部轮播整块空白；无可用来源时立即回退到最近更新或本地兜底图。
- 修复 API 8 规则被旧客户端当作空 XPath 规则执行，导致搜索地址为空、无法播放的问题。
- 修复站点页面改版后绝对章节 XPath 失效的问题；原规则无线路时会从播放列表容器安全回退解析。
- 补齐与 Kazumi Apple 端一致的 Fetch/XHR 响应体探测，可识别 URL 不带 `.m3u8`、但响应内容为 HLS 播放列表的媒体接口。
- 对 GET、HTML 与媒体清单请求增加有上限的瞬时网络重试，缓解超时、连接重置和 TLS 握手中断；证书信任失败不会被重试或绕过。
- 将后台 WebKit 解析器放入独立的非主窗口，避免把 `WKWebView` 直接挂到 SwiftUI 宿主视图产生运行时层级警告。
- 延长实播验证的起播观察窗口，避免网络首缓冲较慢时产生假失败。

## v0.2.0 - 2026-05-14

### 新增

- 恢复 DanDanPlay 弹幕功能，支持通过 Bangumi ID 匹配分集弹幕。
- 播放器新增弹幕开关、加载状态提示与 DanDanPlay API 未配置提示。
- 新增源码构建用的本地 DanDanPlay 凭据配置模板，真实凭据不会进入 Git 仓库。
- 增加弹幕海量模式、去重、显示区域和持续时间等设置项。

### 修复

- 优化滚动弹幕动画，减少因播放器时间低频更新导致的跳动和闪烁。
- 稳定滚动弹幕轨道分配，避免同一条弹幕移动中换轨。
- 改善 DanDanPlay 弹幕解析的容错能力。

### 说明

- DanDanPlay Open API 需要用户在源码构建时配置自己的 AppId 与 AppSecret。
- 未配置 DanDanPlay 凭据时，播放器仍可正常播放视频，但弹幕加载会显示未配置提示。

## v0.1.0 - 2026-05-08

KazumiTV 的首个公开开源版本。

### 新增

- Swift 与 SwiftUI 实现的 Apple TV/tvOS 客户端基础体验。
- 推荐、时间表、追番、历史、搜索、详情与播放等主要页面。
- 基于 Fuzi 的 XPath 插件解析能力。
- 使用 SQLite.swift 与 UserDefaults 的本地收藏、历史和设置存储。
- Python Flask + Playwright 服务器代理，用于在 tvOS 环境下解析播放地址。
- 设置页服务器代理配置与连接测试。
- 设置页版本信息，自动读取应用版本号与构建号。
- GitHub Actions CI，覆盖 tvOS 构建和 Python 代理服务检查。
- 开源项目基础文件：GPL-3.0 许可证、NOTICE、README、Issue 模板、PR 模板和 Dependabot 配置。

### 修复

- 改善设置页服务器地址输入框在 tvOS 聚焦状态下的文字可读性。
- 修复服务器地址输入时键盘被焦点变化自动关闭的问题。
- 避免文本框更新期间修改 SwiftUI 状态导致的运行时警告。

### 说明

- 本项目是 [Kazumi](https://github.com/Predidit/Kazumi) 的独立 Apple TV/tvOS 移植版本，已获得原作者同意，但并非由 Kazumi 原作者或原维护团队开发、维护或发布。
- KazumiTV 不托管、不分发、也不提供任何媒体内容。插件规则、数据源及相关网络请求均由用户自行配置和承担责任。
